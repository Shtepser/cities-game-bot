...
START_PAGE = "https://ru.wikinews.org/wiki/Категория:Города_по_алфавиту"
...
def main():
    ...
    with open("../cities-world.csv", 'w') as f:
        writer = csv.writer(f)
...
def link_to_next_page(page: BeautifulSoup) -> str:
    links = page.find_all(attrs={"title": "Категория:Города по алфавиту"})
    for link in links:
